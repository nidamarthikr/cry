...
#'
#' @param stringa A character string.
#' @return A vector of character strings. These strings are the individual words
#'   in the original string, without blanks.
#' @examples
#' sin <- "My name is nobody."
#' csout <- .tagRead(sin)
#' print(csout)
#' @export
...
#' stamp
#'
#' @param f An object of class "connection". This is normally generated with the
#'   \code{file} function to create a connection to a file.
#' @return A named list of length 3. The first element of the list is called
...
#' information included in the header. This function returns a list with data
#' and
#'
#' @param f An object of class "connection". This is normally generated with the
#'   \code{file} function to create a connection to a file.
#' @param mesages A logical variable. TRUE means to print out messages after
#'   the function has completed its action; FALSE suppresses the print.
#' @return A named list of length 2. The first element of the list is called
#'   l_data and is another list with as many elements as the various fields
...
#' filename <- "/path/to/an/mtz/file.mtz"
...
#' errF <- irdata[[1]]
#' if (errF != 0)
#' stop("readMTZ: MTZ binary file does not contain initial \"MTZ \" tag.
#' It is either a badly formatted or a wrong MTZ file.")
#'
#' # Work out how many reflection records are contained in this MTZ file
#' (= nref X ncols)
#' numDataItems <- irdata[[2]] - 20 - 1
#'
#' # Load all reflection records (only use here is for getting to the right
#' point of binary file)
#' if (irdata[[3]] == 1) reflnData <- readBin(f, "numeric", n=numDataItems,
#' size=4)
#' if (irdata[[3]] == -1) reflnData <- readBin(f, "numeric", n=numDataItems,
#' size=4,endian="big")
#'
...
#' X 3, which I can't find in the binary.
#'
#' @param f An object of class "connection". This is normally generated with the
#'   \code{file} function to create a connection to a file.
#' @return A list with as many elements as the number of batches (images)
#'    included in the mtz file. Each list element is, itself a named list
#'    with all the useful variables stored in batch headers.
#' @examples
#' \dontrun{
#' # Create a connection to binary file
#' filename <- "/path/to/an/unmerged/mtz/file.mtz"
#' f <- file(filename, open="rb")
#'
#' # Reads initial record
#' irdata <- .readIR(f)
...
#' Write data, header and batch headers into an mtz-format file
#'
#' This function takes a list, a file name and a title string as input and
#' writes reflection data and headers information to a new mtz file. The list
#' has the same fields as the list produced by the function ".readMTZ". This
#' makes it easy to carry out operations involving some modification of
#' existing mtz files (see examples).
#'
#' @param data A named list with 3 elements. This is of the type produced by
#'    function ".readMTZ". The first element is called "reflections"
#'    and is a dataframe with as many columns as are included in the mtz file.
#'    The name of each column of the dataframe coincides with the name of the
#'    corresponding column in the mtz. The second element is called "header"
#'    and is a named list in which each name correspond to a valid field in
...
#'    list with as many elements as the number of batches (images)
#'    included in the mtz file. Each list element is, itself, a named list
#'    including all the useful variables stored in batch headers. When working
#'    with merged mtz files the batch_header element is NULL.
#' @param filename A character string. The path to a valid mtz file.
#' @param title A character string. This is the title associated with the mtz
#'    file. Default value is "Untitled", thus potentially different from the
#'    title that might be contained in data previously loaded. This feature
#'    encourages users to always provide sensible titles.
#' @return This function does not return anything as its main effect is the
#'    creation of mtz files.
#' @export
.writeMTZ <- function(data,filename,title="Untitled")
{
 ...
 storage.mode(headerLoc) <- "integer"  # Turn headerLoc into integer for
 ...
 # Fill with NULLs for 69 bytes (2 to complete machine stamp, plus 68 to get
 # to a total of 80. We write one less as this is automatically added when
 # connection is closed)
 for (i in 1:69) writeBin(rawNULL,f)

 # Data (data frame -> matrix -> transpose(matrix) -> vector -> write to
 # binary file)
 ...
 {
  linea <- c()
  linea <- c(linea,data[[2]]$COLUMN$labels[i],data[[2]]$COLUMN$types[i],
             data[[2]]$COLUMN$min[i],data[[2]]$COLUMN$max[i],
             data[[2]]$COLUMN$id[i])
 ...
 {
  hdata <- sprintf("PROJECT%8d %-64s",data[[2]]$PROJECT$id[i],
                   data[[2]]$PROJECT$pname[i])
  fullhdata <- paste(fullhdata,hdata,sep="")
  hdata <- sprintf("CRYSTAL%8d %-64s",data[[2]]$CRYSTAL$id[i],
                   data[[2]]$CRYSTAL$cname[i])
  fullhdata <- paste(fullhdata,hdata,sep="")
  hdata <- sprintf("DATASET%8d %-64s",data[[2]]$DATASET$id[i],
                   data[[2]]$DATASET$dname[i])
  fullhdata <- paste(fullhdata,hdata,sep="")
  hdata <- sprintf("DCELL%10d %10.4f%10.4f%10.4f%10.4f%10.4f%10.4f    ",
                   data[[2]]$DCELL$id[i],data[[2]]$DCELL$a[i],
  ...
  hdata <- sprintf("DWAVEL%9d %10.5f                                                      ",
                   data[[2]]$DWAVEL$id[i],data[[2]]$DWAVEL$lambda[i])
  fullhdata <- paste(fullhdata,hdata,sep="")
 }
 ...
 while (plength > 0)
 {
  ...
  if (plength >= 12)
  {
   for (i in istart:iend) hdata <- paste(hdata,sprintf("%6d",data[[2]]$BATCH[i]),
                                         sep="")
  ...
  if (plength < 12)
  {
   for (i in istart:length(data[[2]]$BATCH)) hdata <- paste(hdata,
                              sprintf("%6d",data[[2]]$BATCH[i]),sep="")
   iend <- 12-rem
 ...
 if (data[[2]]$NCOL[3] != 0)
 {
  ...
  {
   hdata <- sprintf("BH %8d%8d%8d%8d                                             ",
                    num,data[[3]][[num]]$NWORDS,data[[3]][[num]]$NINTGR,
                    data[[3]][[num]]$NREALS)
   ...
   if (!is.null(data[[3]][[num]]$GONLAB))
   {
    ...
    for (i in 1:lengthGONLAB) batchData <- paste(batchData,sprintf("%-9s%-9s%-9s",
            data[[3]][[num]]$GONLAB[1],data[[3]][[num]]$GONLAB[2],
            data[[3]][[num]]$GONLAB[3]),sep="")
    nblanks <- 80-nchar(hdata)
